...
public class TypeTimeListCommand extends CaptureSubCommand {
    ...
    protected String execute(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        sender.sendMessage("ID - Time - Action");
        int id = 0;
        for(TimeOfDay time : type.get().getTimes()) {
            BaseComponent delAction = new TextComponent("[Delete]");
            delAction.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/capture type timeremove " + type.get().getName() + " " + id));
            
            BaseComponent base = new TextComponent(Integer.toString(id));
            base.addExtra(" - ");
            base.addExtra(time.toString());
            base.addExtra(" - ");
